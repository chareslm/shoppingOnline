...
订单状态机：`0` 待支付 → `1` 已支付（待发货）→ `2` 已发货（待收货）→ `3` 已完成（确认收货）；`4` 已取消；`6` 退款中（申请退款后由 `1`/`2` 进入）。超时未支付的订单由定时任务自动关闭为 `4`（取消）。
...
结算当前用户**全部勾选**的购物项，按商家拆单，每个商家生成一个订单。下单时对商品执行库存预占（reserve），预占失败则整单失败并回滚。
...
无勾选购物项返回 HTTP `400` / `40010`；库存不足返回 HTTP `400` / `40011`。下单成功后勾选购物项从购物车移除。
