...
统计模块不把 Redis、Elasticsearch 或前端埋点作为金额、订单和退款的权威来源。第一阶段已使用 MySQL 权威表完成只读精确聚合、人工 SQL 对账、平台／商家概览和日趋势；来源模块契约与容量需求进一步稳定后，再通过标准业务事件生成按日、平台和店铺维度的聚合读模型。分钟级看板允许最终一致，财务对账必须使用离线精确计算。
