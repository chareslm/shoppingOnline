...
- `merchant_application`：入驻申请、主体和联系人信息、双阶段审核结果、邮件投递状态。
...
本期账号审核通过后直接创建 `OPEN` 店铺；暂停、冻结和注销管理接口后续在店铺管理能力中实现。
...
V8 新增 `merchant:qualification:audit`，授予 `ADMIN` 和 `SUPER_ADMIN`。资质审核、账号审核、材料下载和开通邮件重试均使用该权限并记录审计日志。
