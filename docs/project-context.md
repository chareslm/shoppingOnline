...
- 当前集成分支：`develop`；认证权限、强密码与本人改密、管理端认证及用户角色管理、Redis/Elasticsearch 本地 Compose、用户资料/地址/偏好、设备与会话、审计日志、商品/搜索/评价、交易后端及用户 Web 商品/交易页面均已集成。统计模块的指标口径、查询权限、数据范围、事件与聚合边界准备基线已完成；商家、商品和交易前置契约经本轮集成修复后已满足准入，可进入 MySQL 只读精确查询阶段。最新集成提交以远程 `origin/develop` 为准，`main` 尚未包含这些功能。
